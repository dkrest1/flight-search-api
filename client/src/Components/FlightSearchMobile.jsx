...
const FlightSearchMobile = () => {
  const { flightData, addFlight, getPassengers } = useFlightStore();
  ...
  useEffect(() => {
    if (departureOption) {
      setSearchInputs((prevValues) => ({
        ...prevValues,
        origin: departureOption.value,
      }));
    }
    if (arrivalOption) {
      setSearchInputs((prevValues) => ({
        ...prevValues,
        destination: arrivalOption.value,
      }));
    }
  ...
  const verifyInputs = () => {
    ...
    if (
      departureOption &&
      arrivalOption &&
      departureOption.value === arrivalOption.value
    ) {
  ...
  const handleSearchFlight = (event) => {
    ...
    if (Object.keys(validated).length === 0) {
      ...
      axios
        .post(
          "https://flight-search-api.onrender.com/flight/search",
          searchInputs,
          { headers }
        )
        .then((response) => {
          ...
          if (data.length === 0) {
            let text ="No flight reaults for the options selected"
            notify(text)
        ...
        .catch((error) => {
          console.log(error);
          setIsPending(false);
          let text ="Error! something happened";
          notify(text)
        });
    } else {
      setIsPending(false);
      setErrors(validated);
      let text ="Error! please check inputs again";
      notify(text)
    }
  };
          ...
          <div className="w-full flex flex-row rounded-lg divide-2 border-2 shadow-sm h-ful">
            ...
            <div className="w-[43%] flex flex-row border-dashed border-r border-slate-300 h-full items-center justify-center px-2 md:gap-10 py-2 md:py-7">
              ...
              <div className="flex flex-col items-center gap-2 md:-mt-2 ">
                ...
                />
                <small className="text-[9px] text-center text-slate-500 font-medium">
                  {"flight no. " + value.flightNumber}
                </small>
              </div>
              <div className="flex flex-col items-center">
                <p className="font-semibold text-[17px] md:text-sm tracking-normal md:tracking-wide">
            ...
            <div className="flex flex-row w-[43%] justify-between md:justify-end items-center md:gap-32 md:mx-5  ">
              <div className="flex flex-col w-full items-center gap-1 ">
                ...
                <h3 className="text-[16px] md:text-3xl font-semibold tracking-wide -ml-">
                  {"€ " + value.price}
                </h3>
                <small className="text-[9px] md:text-xs text-slate-500 font-medium">
              ...
              <div className="flex flex-col w-full items-end md:items-center md:gap-2">
                <button className="bg-blue-950 text-slate-200 text-[11px] md:text-sm w-fit py-1 md:py-2 px-1 md:px-3 rounded-lg">
                  Book Now
                </button>
      ...
      <form onSubmit={handleSearchFlight} className="w-full flex flex-col">
        ...
        <div className="relative text-sm mt-2">
          ...
          <DropDownSearch
            ...
            setSelectedOption={setDepartureOption}
            options={options}
            setErorrs={setDepartureErorr}
            className=""
          />
          {errors && errors.departureOption && (
            <small className="text-[10px] text-red-600 ml-1">
              {errors.departureOption}
            </small>
          )}
        </div>
        <div className="relative text-sm mt-2">
          ...
          <DropDownSearch
            ...
            setSelectedOption={setArrivalOption}
            options={options}
            setErorrs={setArrivalErorr}
            className=""
          />
        ...
        <button
          className={`text-white bg-blue-950 font-medium text-lg rounded tracking-wider mt-3 py-2 ${
        ...
        >
          {isPending ? (
            <span className="">
              searching <PulseLoader color="#ffffff" size={4} />{" "}
            </span>
          ) : (
            <span>Search</span>
          )}
